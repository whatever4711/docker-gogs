...
QEMU_STATIC = https://github.com/multiarch/qemu-user-static/releases/download/v2.9.1-1
IMAGE = golang:alpine
IMAGE2 = alpine
...
push:
	...
	@$(foreach arch,$(ARCHITECTURES), docker plugin push $(REPO):linux-$(arch)-$(TAG);)
	@docker logout
...
define goarch
	$(shell echo $(1) | sed -e "s|arm32.*|arm|g" -e "s|arm64.*|arm64|g" )
endef
